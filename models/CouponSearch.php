...
/**
 * CouponSearch represents the model behind the search form about `amilna\yes\models\Coupon`.
 */
class CouponSearch extends Coupon
{
    ...
    {
        return [
            [['id', 'qty', 'status', 'isdel'], 'integer'],
            [['code', 'description', 'price', 'discount', 'time_from', 'time_to', 'time'/*, 'redeemsId'*/], 'safe'],
        ];
    }
	...
	{
		return parent::find()->where([Coupon::tableName().'.isdel' => 0]);
	}
	*/
    ...
    public function search($params)
    {
		...
		$dataProvider->sort->attributes['redeemsId'] = [			
			'asc' => ['{{%redeems}}.id' => SORT_ASC],
			'desc' => ['{{%redeems}}.id' => SORT_DESC],
		];*/
        ...
        $params = self::queryNumber([['id',$this->tableName()],['qty'],['status'],['isdel']/*['id','{{%redeems}}']*/]);
		foreach ($params as $p)
		{
			$query->andFilterWhere($p);
		}
        $params = self::queryString([['code'],['description'],['price'],['discount']/*['id','{{%redeems}}']*/]);
		foreach ($params as $p)
		{
			$query->andFilterWhere($p);
		}
        $params = self::queryTime([['time_from'],['time_to'],['time']/*['id','{{%redeems}}']*/]);
		foreach ($params as $p)
		{
			$query->andFilterWhere($p);
		}		
